...
public class JogoDoAdivinhaFragment extends Fragment {
    ...
    public void salvarJogada(int idJogador, int numeroDeTentativas)
    {
        Jogador jogador = new Jogador().selectByPK(idJogador);

        if (jogador == null) // Caso em que ainda não há registros do jogador
        {
            jogador = new Jogador();
            jogador.setId(idJogador);
            jogador.setNome("" + idJogador);
            jogador.setNumeroDeTentativas(numeroDeTentativas);
            jogador.insert();
        }

        // Se há registros, checa se a pontuação melhorou
        else if (numeroDeTentativas < jogador.getNumeroDeTentativas())
        {
            jogador.setNumeroDeTentativas(numeroDeTentativas);
            jogador.update();
        }
...
//        Persistence.save("" + jogador, numeroDeTentativas);
        Persistence.save("last_player", idJogador);
        MainActivity.ultimoJogador = idJogador;
